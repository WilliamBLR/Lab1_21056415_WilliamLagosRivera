...
;Dom: tipoCarta x nombre (string) x resto
;Rec: lista (representacion del TDA Carta) o #f 
  ...
  (cond
    ...
    [(equal? tipoCarta 'pokemon)
     (if (and (= (length resto) 9)
         ...
         ;implementacion para el RF11, campos para energia daño y estado
...
;Funcion que tan solo verifica que esto tenga la estructura soicitada
(define (card? c)
  (and (list? c)
       (>= (length c) 4) 
       (equal? (car c) 'card)
...
;Cada extractor trae su nombre, por temas de tiempó no ire explicando cada uno de ellos, se puede asumir
